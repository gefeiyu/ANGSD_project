...
PGRP <- plotCounts(DESeq.ds.c1c2, gene="PGRP-SB1",intgroup = c("condition","timepoint"), normalized = TRUE, returnData=TRUE)
PGRP$condition_timepoint <- paste(PGRP$condition, PGRP$timepoint, sep="_")

PGRP_p <- ggplot(PGRP, aes(x = condition_timepoint, y = count, color = condition_timepoint)) + 
  geom_point(position=position_jitter(w = 0.1,h = 0),size = 5) + 
  theme_bw() +
  ggtitle("Peptidoglycan recognition SB1 (PGRP-SB1)") +
  ylab("Normalized Count")+
  xlab(" ")+
  scale_x_discrete(guide = guide_axis(n.dodge = 2))+
  expand_limits(y = 0)+ ylim(0,200)+
  theme(plot.title = element_text(hjust = 0.5))+ theme(plot.title = element_text(size=21, face='bold'))+
...
mtk <- plotCounts(DESeq.ds.c1c2, gene="Mtk",intgroup = c("condition","timepoint"), normalized = TRUE, returnData=TRUE)
mtk$condition_timepoint <- paste(mtk$condition, mtk$timepoint, sep="_")

mtk_p <- ggplot(mtk, aes(x = condition_timepoint, y = count, color = condition_timepoint)) + 
  geom_point(position=position_jitter(w = 0.1,h = 0),size = 5) + 
  theme_bw() +
  ggtitle("Metchnikowin (Mtk)") +
  ylab("Normalized Count")+
  xlab(" ")+
  scale_x_discrete(guide = guide_axis(n.dodge = 2))+
  expand_limits(y = 0)+ ylim(0,200)+
...
dptA <- plotCounts(DESeq.ds.c1c2, gene="DptA",intgroup = c("condition","timepoint"), normalized = TRUE, returnData=TRUE)
dptA$condition_timepoint <- paste(dptA$condition, dptA$timepoint, sep="_")

dptA_p <- ggplot(dptA, aes(x = condition_timepoint, y = count, color = condition_timepoint)) + 
  geom_point(position=position_jitter(w = 0.1,h = 0),size = 5) + 
  theme_bw() +
  ggtitle("Diptericin A (DptA)") +
  ylab("Normalized Count")+
  xlab(" ")+
  scale_x_discrete(guide = guide_axis(n.dodge = 2))+
  expand_limits(y = 0)+
...
dptB <- plotCounts(DESeq.ds.c1c2, gene="DptB",intgroup = c("condition","timepoint"), normalized = TRUE, returnData=TRUE)
dptB$condition_timepoint <- paste(dptB$condition, dptB$timepoint, sep="_")

dptB_p <- ggplot(dptB, aes(x = condition_timepoint, y = count, color = condition_timepoint)) + 
  geom_point(position=position_jitter(w = 0.1,h = 0),size = 5) + 
  theme_bw() +
  ggtitle("Diptericin B (DptB)") +
  ylab("Normalized Count")+
  xlab(" ")+
  scale_x_discrete(guide = guide_axis(n.dodge = 2))+
  expand_limits(y = 0)+
...
#png("NFkb_gene_count.png",width=1500, height=900)
grid.arrange(PGRP_p, mtk_p, dptA_p, dptB_p, ncol = 2)
#dev.off()
```
...
![](/Users/verayu/Desktop/ANGSD/NFkb_gene_count.png)
